class PostsTagsAddForeignKeys < ActiveRecord::Migration
  def self.up
    transaction do
      ...
      begin
        execute "alter table posts_tags add constraint fk_posts_tags__post (post_id) references posts on delete cascade"
      rescue Exception
      end

      begin
        execute "alter table posts_tags add constraint fk_posts_tags__tag (tag_id) references tags on delete cascade"
      rescue Exception
      end
    end
  end
