class PostTagHistoryController < ApplicationController
  ...
  def index
    ...
    @changes = PostTagHistory.all(PostTagHistory.generate_sql(params).merge(:order => "id DESC", :limit => 250))
  ...
  def revert
    ...
    if request.post?
      @post.update_attributes(:updater_ip_addr => request.remote_ip, :updater_user_id => @current_user.id, :tags => @change.tags, :rating => @change.rating, :parent_id => @change.parent_id, :source => @change.source)
