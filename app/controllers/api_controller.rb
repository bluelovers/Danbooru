		...
		if params["url"]
			...
			foo.each do |t|
				if tag_type.nil?
					...
					@tags += Tag.find(:all, :conditions => ["id IN (SELECT tag_id FROM posts_tags WHERE post_id IN (SELECT post_id FROM posts_tags WHERE tag_id = (SELECT id FROM tags WHERE name = ?))) AND tag_type = ?", Tag.to_aliased(t), tag_type], :order => "name", :select => "name", :limit => 75).map {|i| i.name}
				end
			end
		else
			@tags = []
