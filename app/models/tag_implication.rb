class TagImplication < ActiveRecord::Base
  ...
  def approve(user_id, ip_addr)
    ...
    Post.find(:all, :conditions => "tags_index @@ to_tsquery('danbooru', E'#{Post.generate_sql_escape_helper(t.name)}')").each do |post|
      post.reload
  ...
  def to_xml(options = {})
    {:id => id, :consequent_id => consequent_id, :predicate_id => predicate_id, :pending => is_pending}.to_xml(options.merge(:root => "implication"))
  end
