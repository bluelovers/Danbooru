class FavoriteTag < ActiveRecord::Base
  ...
  def self.find_post_ids(user_id, favtag_name, limit = 60)
    if favtag_name
      find(:all, :conditions => ["user_id = ? AND name ILIKE ? ESCAPE E'\\\\'", user_id, favtag_name.to_escaped_for_sql_like + "%"], :select => "id, cached_post_ids").map {|x| x.cached_post_ids.split(/,/)}.flatten
    else
