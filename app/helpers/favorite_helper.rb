module FavoriteHelper
  def favorite_list(post)
    ...
    if users.empty?
      ...
      html << users.map {|user| link_to(user.pretty_name, :controller => "favorite", :action => "show", :id => user.id)}.join(", ")
    end
  ...
  def print_also_favorited_by(posts, user)
    ...
    s = users.map {|x| link_to(x.pretty_name, :controller => "favorite", :action => "show", :id => x.id)}.uniq.to_sentence
