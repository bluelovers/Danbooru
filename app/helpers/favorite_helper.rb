module FavoriteHelper
  def favorite_list(post)
    ...
    users = post.favorited_by_hash

    if users.empty?
      html << "no one"
    else
      html << users.slice(0, 6).map {|user| link_to(ERB::Util.h(user["name"]), :controller => "user", :action => "show", :id => user["id"])}.join(", ")

      if users.size > 6
        html << content_tag("span", :id => "remaining-favs", :style => "display: none;") do
          ", " + users.slice(6..-1).map {|user| link_to(ERB::Util.h(user["name"]), {:controller => "user", :action => "show", :id => user["id"]})}.join(", ")
        end
