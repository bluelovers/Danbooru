...
class ForumPostTest < ActiveSupport::TestCase
  ...
  def test_parents
    topic = create_forum_post("hello")
    assert_equal(true, topic.is_parent?)
    assert_equal(topic.id, topic.root_id)
    
    resp1 = create_forum_post("orly", topic.id, :creator_id => 2)
    topic.reload
    assert_equal(true, topic.is_parent?)
    ...
    resp2 = create_forum_post("yarly", topic.id, :creator_id => 3)
    topic.reload
